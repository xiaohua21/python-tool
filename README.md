🖥️ 桌面自动化工具
一款功能强大的桌面自动化工具，可以录制和回放鼠标、键盘操作

License Python版本

✨ 主要功能
🎬 录制功能
✅ 鼠标操作录制 - 精确捕捉鼠标移动、点击、滚动
✅ 键盘操作录制 - 完整记录键盘按下和释放事件
✅ 智能时间记录 - 自动记录事件间的时间间隔（每50ms记录一次）
✅ 窗口感知 - 鼠标进入/离开窗口时自动暂停录制
🎯 播放功能
✅ 精确回放 - 完整复现所有录制的操作
✅ 速度调整 - 支持 0.1x 至 10000x 的播放速度调整
✅ 重复播放 - 可设置重复次数（1-99999次）
✅ 暂停/继续 - 播放过程中随时暂停和继续
💾 文件管理
✅ 保存事件 - 将录制的事件保存为 JSON 文件
✅ 加载事件 - 加载之前保存的事件文件进行回放
✅ 多文件支持 - 支持管理多个不同的自动化脚本
🚀 高级特性
✅ 全局快捷键 - 无需激活窗口即可使用快捷键
✅ 高速模式优化 - 高速播放时自动调整参数提高准确性
✅ 详细日志 - 实时显示所有操作日志
✅ 美观界面 - 基于 Tkinter 的现代化用户界面
✅ 线程控制 - 使用多线程实现流畅的UI响应
⌨️ 快捷键说明
快捷键	功能	说明
F9	开始/停止录制	切换录制状态
F10	开始/停止播放	切换播放状态
F11	暂停/继续	在播放过程中暂停或继续
F12	停止所有	停止所有正在进行的操作
ESC	紧急停止	立即停止所有操作并显示提示
🛠️ 技术栈
GUI框架: Tkinter（Python标准库）
鼠标控制: PyAutoGUI + pynput
键盘监听: keyboard + pynput
数据序列化: JSON
并发处理: threading
系统交互: os, time
📋 系统要求
Python 3.6 或更高版本
Windows / macOS / Linux
管理员权限（某些操作可能需要）
📦 安装依赖
# 安装必要的依赖包
pip install pyautogui keyboard mouse pynput

# 或使用 requirements.txt
pip install -r requirements.txt
🚀 快速开始
运行程序
python zidonghua.py
基本使用流程
启动程序 - 运行 python zidonghua.py
按 F9 开始录制 - 程序开始记录你的鼠标和键盘操作
执行操作 - 进行你想自动化的操作序列
按 F9 停止录制 - 完成操作录制
点击保存 - 将录制的事件保存到 JSON 文件
按 F10 开始播放 - 程序将自动重复你的操作
调整播放参数
播放速度: 使用滑块或预设按钮（1x, 10x, 100x, 1000x, 10000x）
重复次数: 在"重复次数"输入框中设置（1-99999）
💡 使用案例
📊 数据处理
批量填充表单
自动化数据输入
重复操作自动执行
🎮 游戏和娱乐
自动化游戏操作
刷怪、采集等重复任务
演示和录屏
🔧 办公自动化
重复的系统操作
批量文件处理
日常工作流自动化
📂 项目结构
python-zc/
├── zidonghua.py       # 主程序文件
├── README.md          # 项目说明文档
├── requirements.txt   # 依赖包列表
└── events/           # 保存的事件文件目录
    └── *.json        # JSON格式的事件文件
⚠️ 注意事项
安全性: 使用 PyAutoGUI 的安全机制，移动鼠标到屏幕角落可紧急停止
权限: 某些系统可能需要管理员权限才能监听全局快捷键
性能: 建议在重复次数过多时使用较高的播放速度
测试: 新建自动化脚本时先用低速度(1x)测试，确认无误后再提速
🔍 日志和调试
程序窗口中的日志面板会显示：

录制/播放的开始和停止
事件数量
操作速度
错误信息和异常
🎓 常见问题
Q: 为什么某些快捷键不起作用? A: 确保程序以管理员身份运行，某些系统可能需要特殊的权限配置。

Q: 播放速度太快会出错怎么办? A: 降低播放速度或检查系统性能。高速模式下已进行了优化，但极限情况下可能需要调整。

Q: 如何加载之前保存的脚本? A: 点击"加载"按钮，选择之前保存的 JSON 文件即可。

🤝 贡献
欢迎提交 Issue 和 Pull Request！如果你有改进建议，请：

Fork 本仓库
创建你的特性分支 (git checkout -b feature/AmazingFeature)
提交你的更改 (git commit -m 'Add some AmazingFeature')
推送到分支 (git push origin feature/AmazingFeature)
开启一个 Pull Request
📄 许可证
本项目采用 MIT 许可证 - 详见 LICENSE 文件

👨‍💻 作者
xiaohua21 - 初始开发和维护
🙏 致谢
感谢所有贡献者和用户的支持
借鉴了 Python 社区的最佳实践
📞 反馈和支持
如有任何问题或建议，欢迎：

在 GitHub 上提交 Issue
查看项目文档和示例代码
用 ❤️ 开发 | Made with ❤️ by xiaohua21
